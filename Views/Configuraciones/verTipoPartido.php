                    ...
                    <table class="table table-dark table-striped text-info fs-5">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?= $row->nombre ?></td>
                                <td>
                                    <a class="btn btn-danger" id="deleteTi"
                                        href="../../Controllers/EstadisticasController.php?c=6"
                                        data-id="<?= $row->id ?>" onclick="obtener(event); return false;">Eliminar</a>
                                </td>
                            </tr>
                            <?php
                                    $pos++;
...
function obtener(event) {
    ...
    Swal.fire({
        title: "¿Estás seguro?",
        text: "Se eliminará el tipo de partido!",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            Swal.fire(
                "¡Eliminado!",
                "Tipo de partido elimanado",
                "success"
            ).then(() => {
                // Redirige a la URL con el ID eliminado
                window.location.href = "../../Controllers/EstadisticasController.php?c=6&id=" + id;
            });
        } else if (result.dismiss === Swal.DismissReason.cancel) {
            Swal.fire(
                "Cancelado",
                "Tipo de Partido No Eliminado",
                "error"
            );
        }
    });
